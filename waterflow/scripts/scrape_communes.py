...
les champs absents d'un prélèvement restent vides.

Chaque commune a un HISTORIQUE de prélèvements (une campagne = un `code_prelevement`).
On stocke jusqu'à MAX_PREL prélèvements réels par commune -> statistiques plus fines.

Sortie : data/communes_eau.csv (une ligne par prélèvement réel)
Option : --load-db  -> insère aussi communes (clients) + prélèvements + mesures + conclusion ARS.
...
PAGE_SIZE = 2500      # nb de résultats récupérés par commune (couvre plusieurs campagnes)
MAX_PREL = 12         # prélèvements max conservés par commune
MIN_PARAMS = 3        # nb minimum de paramètres cibles pour retenir un prélèvement

# Communes réelles (code INSEE, nom, adresse mairie). Un code sans données
# disponibles sur Hub'Eau est automatiquement ignoré.
COMMUNES = [
    ...
    ("06029", "Cannes",             "Place Cornut-Gentille, 06400 Cannes"),
    ("83061", "Fréjus",             "Place Formigé, 83600 Fréjus"),
    ("13103", "Salon-de-Provence",  "Hôtel de Ville, 13300 Salon-de-Provence"),
    ("13004", "Arles",              "Place de la République, 13200 Arles"),
    ("04070", "Digne-les-Bains",    "Boulevard Gassendi, 04000 Digne-les-Bains"),
    ("83069", "Hyères",             "Avenue Joseph Clotis, 83400 Hyères"),
    ("06004", "Antibes",            "Hôtel de Ville, 06600 Antibes"),
    ...
    ("42218", "Saint-Étienne",      "Hôtel de Ville, 42000 Saint-Étienne"),
    ...
    ("30189", "Nîmes",              "Hôtel de Ville, 30000 Nîmes"),
    ...
    ("80021", "Amiens",             "Hôtel de Ville, 80000 Amiens"),
    ...
    ("73065", "Chambéry",           "Hôtel de Ville, 73000 Chambéry"),
    ("26362", "Valence",            "1 Place de la Liberté, 26000 Valence"),
    ("64102", "Bayonne",            "Avenue du Maréchal Leclerc, 64100 Bayonne"),
    ("87154", "Saint-Junien",       "Place Auguste Roche, 87200 Saint-Junien"),
    ("17300", "La Rochelle",        "Hôtel de Ville, 17000 La Rochelle"),
...
# Mapping : feature du modèle -> motif(s) recherché(s) dans le libellé (normalisé)
FEATURE_PATTERNS = {
    "ph":              [lambda x: x == "ph"],
...
def _feature_of(label_norm: str):
...
def fetch_commune(code_insee: str) -> list:
    """Retourne la liste des derniers prélèvements réels d'une commune,
    chacun : {code, date, conclusion, potable, values{feature: valeur}}."""
    r = requests.get(HUBEAU, params={
        "code_commune": code_insee, "size": PAGE_SIZE, "sort": "desc",
    }, timeout=60)
    r.raise_for_status()
    data = r.json().get("data", [])

    prels = {}   # code_prelevement -> dict
    order = []   # ordre d'apparition (déjà trié desc par l'API)
    for row in data:
        cp = row.get("code_prelevement")
        if cp is None:
            continue
        if cp not in prels:
            prels[cp] = {"code": cp, "date": row.get("date_prelevement"),
                         "conclusion": row.get("conclusion_conformite_prelevement"),
                         "values": {}}
            order.append(cp)
        feat = _feature_of(_norm(row.get("libelle_parametre")))
        val = row.get("resultat_numerique")
        if feat and val is not None and feat not in prels[cp]["values"]:
            prels[cp]["values"][feat] = val

    # Ne garder que les prélèvements avec assez de paramètres cibles, les plus récents
    result = []
    for cp in order:
        p = prels[cp]
        if len(p["values"]) >= MIN_PARAMS:
            c = _norm(p["conclusion"])
            p["potable"] = 1 if (p["conclusion"] and "conforme" in c and "non conforme" not in c) else \
                           (0 if p["conclusion"] else None)
            result.append(p)
        if len(result) >= MAX_PREL:
            break
    return result
...
def main():
    ...
    ap.add_argument("--load-db", action="store_true",
                    help="insère aussi communes + prélèvements réels dans la base")
    args = ap.parse_args()
    ...
    all_rows = []           # (dict_ligne, adresse)
    per_commune = []        # (code, nom, adresse, [prelevements])
    print("Extraction Hub'Eau (donnees reelles, historique par commune) :")
    for code, nom, adresse in COMMUNES:
        try:
            prels = fetch_commune(code)
        except Exception as exc:  # noqa: BLE001
            print(f"  ! {nom:22s}: echec ({exc})")
            continue
        if not prels:
            print(f"  - {nom:22s}: aucune donnee exploitable, ignoree")
            continue
        per_commune.append((code, nom, adresse, prels))
        for p in prels:
            row = {"code_insee": code, "commune": nom, "code_prelevement": p["code"],
                   "date_prelevement": p["date"], "conclusion_ars": p["conclusion"],
                   "potable_ars": p["potable"]}
            for feat in FEATURES:
                row[feat] = p["values"].get(feat)
            all_rows.append((row, adresse))
        print(f"  + {nom:22s}: {len(prels)} prelevements reels")

    cols = ["code_insee", "commune", "code_prelevement", "date_prelevement",
            "conclusion_ars", "potable_ars"] + FEATURES
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=cols)
        writer.writeheader()
        for row, _ in all_rows:
            writer.writerow(row)
    print(f"\n[OK] {len(per_commune)} communes / {len(all_rows)} prelevements -> {out_csv}")

    if args.load_db:
        load_into_db(per_commune)


def load_into_db(per_commune):
    import secrets
    ...
    n_clients = 0
    n_prev = 0
    for code, nom, adresse, prels in per_commune:
        id_client = f"INSEE-{code}"
        # Rechargement propre : on supprime le client existant (cascade -> prélèvements)
        existing = db.query(Client).filter_by(id_client=id_client).first()
        if existing:
            db.delete(existing)
            db.commit()

        client = Client(id_client=id_client, denomination=f"Commune de {nom}",
        ...
        for p in prels:
            ...
            prev = Prelevement(client_id=client.id, date_prelevement=dt, created_at=dt,
                               lieu=f"Reseau public - {nom}",
                               source=IngestionSource.MANUAL,
                               observations=p["conclusion"], ocr_warnings="[]")
            db.add(prev)
            db.flush()
            n_prev += 1
            v = p["values"]
            db.add(Mesure(
                ...
                ph=v.get("ph"), hardness=v.get("Hardness"), solids=v.get("Solids"),
                chloramines=v.get("Chloramines"), sulfate=v.get("Sulfate"),
                conductivity=v.get("Conductivity"), organic_carbon=v.get("Organic_carbon"),
                trihalomethanes=v.get("Trihalomethanes"), turbidity=v.get("Turbidity"),
            ))
            if p["potable"] is not None:
                db.add(Prediction(prelevement_id=prev.id, potable=p["potable"],
                                  probability=float(p["potable"]),
                                  model_version="Conclusion sanitaire ARS (donnee reelle)",
                                  created_at=dt))
        db.commit()
    db.close()
    print(f"[OK] Base : {n_clients} communes et {n_prev} prelevements reels inseres.")
